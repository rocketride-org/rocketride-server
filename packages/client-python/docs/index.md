...
| `send_files` | `async def send_files(self, files: List[str \| Tuple[str, dict] \| Tuple[str, dict, str]], token: str, max_concurrent: int = 5) -> List[UPLOAD_RESULT]` | `List[UPLOAD_RESULT]` | Uploads files, at most `max_concurrent` at a time (must be a positive integer). Each item: path `str`, or `(path, objinfo)`, or `(path, objinfo, mimetype)`. Results are in the same order as `files`. Progress via `on_event` as `apaevt_status_upload`. |
