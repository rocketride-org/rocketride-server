...
| Method            | Signature                                                                                                                                                                                                | Returns | Description                                                                                                                                                                                              |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `use`             | `async def use(self, *, token: str = None, filepath: str = None, pipeline: dict = None, source: str = None, threads: int = None, replicas: int = None, torch_threads: int = None, use_existing: bool = None, args: list = None, ttl: int = None) -> dict` | `dict`  | Starts a pipeline. Requires `filepath` or `pipeline`. The client substitutes `${ROCKETRIDE_*}` from its env. Returns a dict with at least `'token'` and `'replicas'`; use the token for all data and control operations. `threads` is the admission width per connection (server default 64) — it does not parallelize inference. `replicas` launches N engine subprocesses behind one token for concurrent inference (default 1, max 32); `torch_threads` sets the per-replica BLAS/OMP thread count (auto: `cores // replicas` when unset and `replicas > 1`). |
