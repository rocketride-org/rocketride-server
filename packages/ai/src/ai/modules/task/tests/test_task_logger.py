...
import logging
import pytest
from ai.modules.task.task_logger import get_task_logger
...
def test_no_duplicate_handlers():
    ...
    get_task_logger('test_engine_dup')
...
#
# NOTE: Task no longer owns a per-instance `self.logger` (that leaked a
# Logger + StreamHandler per task forever - see task_engine.py fix).
# Logging now goes through the shared module-level `_logger` in
# ai.modules.task.task_engine, so we patch that instead of an instance
# attribute.
@pytest.mark.asyncio
async def test_termination_log_level():
    ...
    with patch('ai.modules.task.task_engine._logger') as mock_logger:
        await task_instance._terminated()

        # Assert module-level _logger.warning was called
    ...
    with patch('ai.modules.task.task_engine._logger') as mock_logger:
        ...
        # Assert module-level _logger.info was called
