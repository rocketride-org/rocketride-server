...
class AttachmentDropReport:
    ...
    # User-visible filename, carried so the chat UI can name the dropped file in
    # its notice. Excluded from equality (``compare=False``) so drop-report
    # assertions keyed on provider/mime/reason stay filename-agnostic.
...
def translate_openai_shape(
    ...
    for att in attachments:
        if att.mime in _OPENAI_IMAGE_MIMES:
            ...
            dropped.append(
                AttachmentDropReport(provider=provider_name, mime=att.mime, reason='unsupported', filename=att.filename)
            )
