...
async def test_every_generated_python_file_compiles(catalog_engine):
    ...
    assert len(py) == 3, f'expected the node init, IGlobal and IInstance, got {sorted(py)}'
...
async def test_manifest_carries_the_keys_the_engine_actually_requires(catalog_engine):
    ...
    name a factory type; and the import path is nodes.<name>.
    """
    result = await _scaffold(catalog_engine, name='my_node')

    manifest = json.loads(result['files']['nodes/my_node/services.json'])

    assert manifest['protocol'] == 'my_node://'
    assert manifest['register'] == 'filter'
    assert manifest['path'] == 'nodes.my_node'
...
@pytest.mark.asyncio
async def test_the_parent_carries_no_package_marker(catalog_engine):
    """`nodes` is a namespace package: an __init__.py there would close it.

    A regular package resolves to one directory — the first on sys.path — so a
    scaffolded node would hide every node the engine ships, or be hidden by
    them. Leaving the parent unmarked is what lets both be imported.
    """
    result = await _scaffold(catalog_engine)

    assert 'nodes/__init__.py' not in result['files']
...
async def test_depends_is_called_from_iglobal_not_init(catalog_engine):
    ...
    assert 'depends(' in result['files']['nodes/my_node/IGlobal.py']
    assert 'depends(' not in result['files']['nodes/my_node/__init__.py']
...
async def test_the_handler_matches_the_requested_lane(catalog_engine):
    ...
    instance = result['files']['nodes/doc_node/IInstance.py']
...
async def test_a_name_that_is_not_importable_is_rejected(catalog_engine):
    """The engine imports nodes.<name>, so a bad name fails at load, not here."""
