...
if(VCPKG_TARGET_IS_WINDOWS OR VCPKG_TARGET_IS_UWP)
    ...
    if(PYTHON_ALLOW_EXTENSIONS)
        ...
        find_library(FFI_RELEASE NAMES ffi libffi PATHS "${CURRENT_INSTALLED_DIR}/lib" NO_DEFAULT_PATH)
        find_library(FFI_DEBUG NAMES ffi libffi PATHS "${CURRENT_INSTALLED_DIR}/debug/lib" NO_DEFAULT_PATH)
	...
	#--------------------------------------------------------------------------------
	# We don't
	# need to pick up any other DLLs (like lipcrypto, libffi, libssl, as we have
	# statically linked to those. We will also copy the debug executable and pdb
	# as well
	#--------------------------------------------------------------------------------
    # Copy over the release versions of the python dlls
    if(PYTHON_ALLOW_EXTENSIONS)
		...
		# Copy over the debug versions of the python dlls
		if(PYTHON_ALLOW_EXTENSIONS)
        file(GLOB_RECURSE PYTHON_EXTENSIONS "${CURRENT_BUILDTREES_DIR}/*_d.pyd")
        list(FILTER PYTHON_EXTENSIONS INCLUDE REGEX [[.*_d\.pyd]])
        file(COPY ${PYTHON_EXTENSIONS} DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}/debug/DLLs")
        vcpkg_copy_tool_dependencies("${CURRENT_PACKAGES_DIR}/tools/${PORT}/debug/DLLs")
        file(REMOVE "${CURRENT_PACKAGES_DIR}/tools/${PORT}/debug/DLLs/python${PYTHON_VERSION_MAJOR}${PYTHON_VERSION_MINOR}_d.dll")
    endif()

	# Copy the debug versions of python_d.exe and python310_d.dll
	file(GLOB_RECURSE PYTHON_EXTENSIONS "${CURRENT_BUILDTREES_DIR}/python_d.exe")
	file(COPY ${PYTHON_EXTENSIONS} DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}/debug")

	file(GLOB_RECURSE PYTHON_EXTENSIONS "${CURRENT_BUILDTREES_DIR}/python_d.pdb")
	file(COPY ${PYTHON_EXTENSIONS} DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}/debug")
	#--------------------------------------------------------------------------------
	...
	#--------------------------------------------------------------------------------
	# Do not remove source so we can debug the actual C++ code
	#--------------------------------------------------------------------------------
    # vcpkg_clean_msbuild()
	#--------------------------------------------------------------------------------
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
        set(LIB_PATH_VCPKG ${CURRENT_INSTALLED_DIR}/lib)
    else()
        set(LIB_PATH_VCPKG ${CURRENT_INSTALLED_DIR}/debug/lib)
    endif()

    if(VCPKG_TARGET_IS_OSX)
        list(APPEND OPTIONS "LIBS= -liconv ${LIB_PATH_VCPKG}/libintl.a")
    endif()
	...
	# Remove nis module from failed list (NIS is obsolete and not commonly used)
	# Newer versions of clang (16+ do not automatically include the -lnis library
	# so it fails the whole build.
	list(FILTER python_libs_dynload_failed_debug EXCLUDE REGEX ".*nis\..*_failed\.so.*")
	list(FILTER python_libs_dynload_failed_release EXCLUDE REGEX ".*nis\..*_failed\.so.*")
