...
"""
Benchmark: Chonkie (SIMD-accelerated) chunking.

Runs the same pipeline as bench_langchain.py and bench_rocketride.py
but uses Chonkie's compiled chunkers for a fair compiled-vs-compiled
comparison with RocketRide's C++ nodes.

Usage:
    pip install chonkie[all] psutil
    python benchmarks/bench_chonkie.py <docs_dir>
"""
...
def chunk_with_chonkie_token(docs, chunk_size=512, chunk_overlap=50):
    """Chunk using Chonkie TokenChunker."""
...
def chunk_with_chonkie_recursive(docs, chunk_size=512, chunk_overlap=50):
    """Chunk using Chonkie RecursiveChunker (semantic-aware)."""
...
def run(root_dir):
    ...
    print('=' * 60)
    print('BENCHMARK: Chonkie (compiled chunkers)')
    print('=' * 60)
    ...
    # Stage 3: Chunking with Chonkie TokenChunker
    print('\n[3/6] Chunking with Chonkie TokenChunker (512)...')
    gc.collect()
    mem_before_chunk = get_mem_mb()
    t0 = time.perf_counter()
    chunks = chunk_with_chonkie_token(docs, chunk_size=512, chunk_overlap=50)
    t1 = time.perf_counter()
