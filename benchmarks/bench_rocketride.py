...
def native_chunk_text(text_bytes, chunk_size=512, overlap=50):
    ...
    n = _chunker.chunk_text(text_bytes, text_len, chunk_size, overlap, offsets, max_chunks)
    chunks = []
    for i in range(n):
        start = offsets[i * 2]
...
def run(root_dir):
    ...
    print(f'      {len(entries):,} text files ({total_disk_mb:.0f} MB) in {t_discover:.2f}s')
    ...
    print(f'      {len(docs):,} docs ({text_mb:.0f} MB), {errors} errors, {t_parse:.2f}s')
    ...
    print(f'      {len(all_chunk_bytes) / t_chunk:,.0f} chunks/sec  |  Memory: {mem_chunk:.0f} MB')
    ...
    print(f'      Index memory: {idx_mem_mb:.0f} MB  |  Process memory: {mem_index:.0f} MB')
