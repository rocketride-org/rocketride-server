...
other events carry a `replica` index, present only when `replicas > 1`, so
you can tell which replica produced them.
...
- Each replica is a **full copy of the model in RAM**. Budget memory
  ...
  costs roughly `8 * M`.
- Idle replicas still cost CPU, so don't set `replicas` far above what
  your traffic can actually keep busy.
...
Invalid values fall back to the default rather than failing the server.
...
## Sharing one model instead: `--modelserver`

`replicas` costs N model copies. The engine also accepts
`--modelserver=<PORT|HOST:PORT>`, which it forwards to every task
subprocess so that their model clients proxy inference to a separate
model-serving process instead of loading a private copy. That server is
not part of this repository; `replicas` is the supported option here.

## See also

- [Engine Reference](README-engine.md) -- CLI options and env vars.
