...
1. **Auto-detect + fallback (`ConfigBuilder.getConfig`).** At config-build time the engine probes for the external tools. It keeps the external parsers **only when the full toolchain is present** (`ffmpeg` **and** `exiftool` **and** `sox`, plus `env` on non-Windows); if **any** is missing it excludes `ExternalParser`/`CompositeExternalParser` and falls back to the built-in parsers for everything. This all-or-nothing rule avoids a mixed state where a kept external parser throws for a file whose specific tool is absent. On **Windows** the tools launch via `env` (absent), so the external parsers are always excluded — built-in parsers are always used there.
