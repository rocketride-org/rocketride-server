# Build System Authoring

How to **write** build tasks: the `scripts/tasks.js` contract, actions, control-flow
helpers, deduplication, shared state, utilities, and the patterns that hold up at
scale.

To **run** builds — module list, build commands, output layout, CLI flags, and the
C++ compiler toolchain — see [Build System Reference](reference.md).
...
Once the module is registered, invoke and inspect it with the CLI — `builder
my-package:build`, `--force`, `--verbose`, `--list-actions`, `--list-deps`. Those
flags are documented in
[Build System Reference → CLI Usage](reference.md#cli-usage).
