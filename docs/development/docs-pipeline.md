...
Everything here lives in `docs/docusaurus/`:
...
| `STATIC_DIR` | `docs/docusaurus/static/` | Raw pre-MDX `.md` siblings of every page, for the LLM surface and copy-as-markdown |
...
- **Root mounts.** `DOCS_ROOT_MOUNTS` in `gather.js` maps directories of the
  ...
  | `docs/docusaurus/apps/vscode` | `clients/vscode` |
  | `docs/public/mcp/stdio` | `connect/mcp/stdio` |
  | `docs/public/mcp/http` | `connect/mcp/http` |

The sweep covers **all** of `docs/public/` and `docs/docusaurus/apps/`, so a new `.md` there with no covering
...
`docs/docusaurus/static/<id>.md` — that is the copy-as-markdown and LLM surface.
...
`docs/docusaurus/redirects.ts` is a flat `{ to, from[] }` array wired into
...
Five files are owned by `docs/` but have to physically exist somewhere else,
because npm and PyPI render a package's own `README.md`. `docs:export` copies
them out with a `GENERATED by ./builder docs:export — DO NOT EDIT` header, so
anyone opening the copy knows to edit upstream. Relative image links are
rewritten to raw-GitHub URLs on `main` in the copy; the source keeps
`./assets/...` so it previews on any branch.
...
| `docs/public/chat-widget/README.md` | `packages/chat-widget/README.md` |

`docs:check` runs the identical computation with `check: true`, writes nothing,
and fails listing any destination that was hand-edited or never exported.

The agent docs are not exported. `docs/agents/context/` is packed into
`docs.zip` by `client-docs:agent` (`docs/agents/scripts/tasks.js`), served by
the engine at `GET /client/docs`, and installed into a workspace's
`.rocketride/docs/` by the VS Code extension and `rocketride init`.
