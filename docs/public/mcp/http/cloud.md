...
desktop MCP client, see the [`rocketride-mcp`](/connect/mcp/stdio) bridge package
...
same tools RocketRide's own in-canvas [Canvas Agent](/guides/canvas-agent) uses,
...
[Canvas Agent](/guides/canvas-agent) is the one surface that specifically requires a
