...
The server exposes 33 tools (`send_files` only on a local engine), organized by capability — the full parameter-level
...
| [Manage deployments](/connect/mcp/http/tools#manage-deployments) | `deploy_add`, `deploy_list`, `deploy_status`, `deploy_versions`, `deploy_to_team`, `deploy_set_schedule`, `deploy_enable`, `deploy_disable`, `deploy_remove` |
...
- **Store-scoped reads** — the four `store_*` tools resolve paths through your
  account's file store on the engine, nothing else. (`send_files` reads paths
  on the engine host, so only a loopback-bound local engine offers it — see
  its [reference entry](/connect/mcp/http/tools#send_files).)
...
<a className="rr-side-card" href="/connect/mcp/http/tools">
  ...
  <span className="rr-side-card__body">All 33 tools with parameters, return shapes, and the error contract.</span>
</a>
