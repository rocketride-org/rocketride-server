...
lanes, or control connections. Always [validate](/reference/pipeline-reference) the
...
- [Execution model](/concepts/execution-model): how lanes carry data between
  nodes.
- [Pipeline JSON reference](/reference/pipeline-reference): every field of a node.
