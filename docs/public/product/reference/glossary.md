...
its nodes, and streams data through the graph. Runs locally, [self-hosted](/operate/self-hosting),
or on [Cloud](/operate/cloud). See [Runtime & engine](/concepts/runtime-engine).

## `.pipe` file

The JSON file that defines a pipeline, conforming to the
[Pipeline JSON Reference](/reference/pipeline-reference). The same file runs unchanged in
every environment.
...
assistants. See [MCP](/connect/mcp).
