...
See the [Getting Started](#getting-started) section for the complete `rsbuild.config.ts` template.
...
- `rocketride/app-sdk` must be a shared singleton so your app and the shell agree on one instance
