...
> includes two additional commands (`events`, `list`) not present in
...
| `--token <token>` | Supply a custom task token for the new run (the server generates one if not provided). A pipeline file is still required — to follow a task that is already running, use `status` or `events`. |
