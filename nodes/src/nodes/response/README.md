...
## Live media (media-plane)

When `transmit_media` is on **and** the engine has an SFU configured, the node streams `audio`/`video` lanes progressively over WebRTC instead of waiting for the whole file. It pushes the lane by **RTSP to a MediaMTX SFU**, which re-exposes it as **WHEP**; the announcement then carries `live: true` and the `url` is a WHEP endpoint clients open with an `RTCPeerConnection` (video is copied as-is, audio is transcoded to Opus).

The media-plane is **off unless `ROCKETRIDE_MEDIA_SFU` is set** — an unset value never downloads a binary or opens a port, so a deploy that does not opt in (e.g. a SaaS pod) stays inert and media falls back to normal delivery. The value selects the mode:

- **`managed`** (or `local`) → the engine downloads and runs its own MediaMTX (zero-config for OSS/local dev).
- **`<host>`** (e.g. `sfu.internal`) → an external SFU at that host (SaaS points here).

RTSP goes to `:8554`, WHEP is served at `http://<host>:8889/<stream_id>/whep`.

- **`ROCKETRIDE_MEDIA_WHEP_BASE`** (optional) overrides the base URL clients pull WHEP from, e.g. `https://media.example.com` — the announced url becomes `<base>/<stream_id>/whep`. Set it when a TLS ingress fronts the SFU and the client-facing host or scheme differs from the internal RTSP host: a browser on an https page rejects an http WHEP url. Unset, WHEP falls back to `http://<sfu-host>:8889`.
- **`ROCKETRIDE_MEDIA_JWT_KEY`** (optional) points at a PEM RSA private key; when set, the announced WHEP url carries a `?jwt=` token scoped to read only that stream, which MediaMTX validates (`authMethod: jwt`). Unset, no token is added. The `stream_id` is always an unguessable per-client capability token regardless.
