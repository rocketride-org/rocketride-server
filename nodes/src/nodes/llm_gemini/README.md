...
This node forwards chat attachments to the model in the Gemini `parts` format. When a message carries
attachments, these are forwarded:

| Modality | Forwarded MIME types |
|----------|----------------------|
| Image | `image/png`, `image/jpeg`, `image/webp`, `image/heic` |
| PDF   | `application/pdf` |
