...
and output limit, so the UI requires no additional model settings.
...
Moonshot also publishes `moonshot-v1-{8k,32k,128k}-vision-preview` image-input
models. Those belong in a dedicated vision node in the `llm_vision_*` family and
are not available in this text-only node.
