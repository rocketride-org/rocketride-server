...
This node forwards chat attachments to the model in the AWS Bedrock Converse format. When a message
carries attachments, these are forwarded:

| Modality | Forwarded MIME types |
|----------|----------------------|
| Image | `image/png`, `image/jpeg`, `image/gif`, `image/webp` |
| Documents | `application/pdf`, `text/html`, `text/plain`, `text/markdown`, `text/csv`, `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` (xlsx), `application/vnd.openxmlformats-officedocument.wordprocessingml.document` (docx), `application/msword` (doc) |
| Video | `video/mp4`, `video/quicktime`, `video/x-matroska`, `video/webm`, `video/x-flv` |

Audio is not supported by this provider.

> **Note:** Forwarding only delivers the attachment in the request — whether it is understood depends
> on the specific model you configure (e.g. only vision-capable models read images). Unsupported MIME
