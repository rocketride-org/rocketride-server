...
| `channel_history` | Read recent messages via `conversations.history`, newest first. `channel` is required (the bot must be a member); `limit` 1–200 (default 50); optional `oldest`/`latest` timestamps bound the window. Returns `ts`, `user`, and `text` per message, plus `thread_ts` for threaded messages. Also returns `subtype` whenever Slack classifies the entry, and `bot_id` whenever a bot posted it. `subtype` is a classifier, not a noise flag: `file_share`, `thread_broadcast` and `me_message` are ordinary user messages, while `channel_join`, `channel_leave`, `channel_topic`, `tombstone`, `pinned_item` and `reminder_add` are system entries — that system list is open-ended, so drop entries by naming the subtypes you do not want rather than by presence. `bot_id` is the reliable way to skip your own posts: compare it against the one `check_connection` reports. Do not filter on the `bot_message` subtype — an app posting under its own bot identity produces a `bot_id` and no subtype at all. |
