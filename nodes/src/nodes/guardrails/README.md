...
## Guarding tool calls (control-plane attachment)

Lane connections only ever see whatever feeds a pipeline's final response — they can't see what an agent does with a tool (`tool_slack`, `tool_gmail`, `tool_github`, a database-as-tool, etc.), because tool calls happen entirely inside the agent's own turn. To guard those, attach `guardrails` to an agent node via `control` instead of a lane:

```json
{
  "id": "guardrails_1",
  "provider": "guardrails",
  "config": { "type": "guardrails", "policy_mode": "block" },
  "control": [
    { "classType": "guard", "from": "agent_rocketride_1" }
  ]
}
```

One guard node covers every tool the agent holds — no per-tool wiring needed. For each tool call, the same `GuardrailsEngine` runs twice:
