...
<head>
  <title>VertexAI (Enterprise) - RocketRide Documentation</title>
</head>

## What it does

Connects Google Cloud Vertex AI models to your pipeline using service account authentication. Supports Gemini, Claude, Gemma, Llama, and DeepSeek models hosted on Vertex AI. Used primarily as an `llm` invoke connection by agents and other nodes that need an LLM. Can also be used directly via lanes.

Requires an enterprise plan. For personal Google account access, use [VertexAI (Personal)](../vertex-personal/).
...
| Field       | Description                                 |
| ----------- | ------------------------------------------- |
| Customer ID | Google Workspace customer ID                |
| Admin Email | Service account admin email                 |
| Service Key | Service account JSON key                    |
