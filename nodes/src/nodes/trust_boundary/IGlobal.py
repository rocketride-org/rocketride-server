    ...
    def beginGlobal(self):
        ...
        for scope_def in raw_scopes[:50]:  # Max 50 scopes
            ...
            if scope.max_calls_per_run < 0:
                warning(f"[TrustBoundary] max_calls_per_run must be >= 0 for scope '{scope.scope_id}'; using 0")
        ...
        try:
            ...
            if config.enable_run_policy and config.payload_schema:
                warning(
                    '[TrustBoundary] jsonschema library not available; run-level payload schema enforcement disabled'
                )
