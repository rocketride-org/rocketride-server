...
3. The manager builds an `Agent + Task` per sub-agent, routing LLM and tool calls back through that sub-agent's own pipeline channels. Each delegate task is then given the manager's own `tool` channel in place of the tools it inherited from its sub-agent. That single assignment does two things: a sub-agent's tools never reach the manager, so delegation always resolves to that sub-agent's own tools; and the manager's cross-cutting tools are available to it on every task it executes, without being routed to any sub-agent. Delegation is unaffected — CrewAI merges the delegation tools on top rather than replacing them. The manager agent itself is deliberately built with **no** tools, because CrewAI rejects a hierarchical manager that carries any
