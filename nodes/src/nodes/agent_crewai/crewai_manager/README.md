...
3. The manager builds an `Agent + Task` per sub-agent, routing LLM and tool calls back through that sub-agent's own pipeline channels. Each delegate task is built with no tools of its own, so delegation always resolves to that sub-agent's tools, never the manager's — the manager's own `tool` channel (if wired) is separate and available to it on every task it executes, not routed to any sub-agent
