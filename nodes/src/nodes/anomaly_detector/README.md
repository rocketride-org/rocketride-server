...
| Method | Description |
|---|---|
| `beginGlobal(self)` | Initialize the anomaly detector when the pipeline starts. |
| `endGlobal(self)` | Clean up the anomaly detector when the pipeline ends. |
...
| `writeText(self, text: str)` | Process text and check for anomalous numeric values. |
| `writeDocuments(self, documents: List[Doc])` | Process documents and check metric fields for anomalies. |
| `closing(self)` | Finalize any pending operations before close. |
