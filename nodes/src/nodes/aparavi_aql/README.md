...
| Method | Description |
|---|---|
| `beginGlobal(self) -> None` | Initialize the Aparavi HTTP client from node configuration. |
| `endGlobal(self) -> None` | Release the HTTP client. |
...
| Method | Description |
|---|---|
| `get_data(self, args: Any) -> Dict[str, Any]` | Translate natural language to AQL, execute against Aparavi, and return rows. |
| `get_aql(self, args: Any) -> Dict[str, Any]` | Generate an AQL SELECT statement from natural language without executing it. |
| `get_schema(self, args: Any) -> Dict[str, Any]` | Return the fixed STORE table column schema. |
