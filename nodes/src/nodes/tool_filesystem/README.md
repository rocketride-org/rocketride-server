...
(`users/<client_id>/files/`, the same storage area the client SDK reaches via its
`fs_*` methods) and all sharing this node's code (`nodes/src/nodes/tool_filesystem/`):
...
The account is resolved automatically from the `ROCKETRIDE_CLIENT_ID` env var injected
by the task engine; no account configuration is needed on any of the three.
...
by the agent are visible to the client SDK and vice versa. If `ROCKETRIDE_CLIENT_ID`
is missing or the account store fails to initialise, a warning is logged and **all**
