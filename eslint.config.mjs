...
export default tseslint.config(
	// Global ignores
	{
		ignores: [
			...
			'packages/n8n-nodes/**',
			// Frozen API contract floors — generated by shell:freeze / client-typescript:freeze, immutable (see .gitattributes).
			'packages/shell/contract/versions/**',
			'packages/client-typescript/contract/versions/**',
		],
	},
	...
	// Plain .js/.cjs files in this repo are CommonJS (no package sets
	// "type": "module"): tool configs (postcss/tailwind/jest), stubs, scripts.
	// Environment only - no rule changes. (sourceType is left at the default:
	// with 'commonjs', no-redeclare treats top-level `const crypto = require(..)`
	// as a redeclaration of the builtin global instead of a module-local.)
	{
		files: ['**/*.{js,cjs}'],
		languageOptions: {
			globals: {
	...
	// .mjs files are Node ESM (build configs, generators, tests).
	{
		files: ['**/*.mjs'],
		languageOptions: {
			globals: {
	...
	// Hooks also live in plain .ts files: register the react-hooks plugin
	// there so its rules run (and so its eslint-disable directives resolve).
	{
		files: ['**/*.{ts,mts}'],
		plugins: {
			'react-hooks': reactHooksPlugin,
