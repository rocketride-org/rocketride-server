...
async function captureVsEnvironment(vsRoot, options = {}) {
    ...
    try {
        const result = await execCommand('cmd', ['/c', vsvarsPath, vsRoot], { cwd: PROJECT_ROOT, collect: true, verbose, onOutput });
        const env = {};
        ...
        const hasVsEnv = (env.INCLUDE && (env.INCLUDE.includes('VC') || env.INCLUDE.includes('Windows'))) ||
...
async function installVs(log) {
    try {
        ...
        await execCommand(bootstrapperPath, [
            '--quiet', '--wait',
            '--add', 'Microsoft.VisualStudio.Workload.VCTools', // install by workload; detection uses components above
            '--includeRecommended',
            '--add', VS_REQUIRES_ATL,
            '--add', VS_REQUIRES_CMAKENINJA
