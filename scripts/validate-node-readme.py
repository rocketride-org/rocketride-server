#!/usr/bin/env python3
"""Validate node READMEs against docs/internal/node-readme-schema.md.
...
def load_services(node_dir: Path):
    ...
    for e in entries[1:]:
        merged["classType"] = sorted(set(merged.get("classType") or []) |
                                     set(e.get("classType") or []))
        merged["capabilities"] = sorted(set(merged.get("capabilities") or []) |
                                        set(e.get("capabilities") or []))
        ...
        pc_m["profiles"] = {**(pc_e.get("profiles") or {}),
                            **(pc_m.get("profiles") or {})}
...
def table_col(text: str, section: str, col: int = 0):
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r"^\|[\s\-:|]+\|$", nxt):
            continue  # header row
        cells = [re.sub(r"\s*\(default\)\s*", "", re.sub(r"[*_`]", "", c)).strip()
...
def validate(node_dir: Path):
    ...
    if GEN_START in full:
        ...
        add("PASS" if GEN_END in tail else "FAIL",
            "generated region is terminated", "missing END marker")
        after = tail.split(GEN_END, 1)[1] if GEN_END in tail else ""
        add("PASS" if not after.strip() else "FAIL",
            "generated region is last", "content found after END marker")
    else:
        hand = full
    ...
    add("PASS" if m and m.group(1) == name else "FAIL",
        "H1 equals directory name",
        f"'{m.group(1) if m else '(no H1)'}' != '{name}'"
        if not (m and m.group(1) == name) else "")
    if m:
        rest = hand[m.end():]
        first_para = next((p.strip() for p in rest.split("\n\n") if p.strip()), "")
        add("PASS" if first_para and not first_para.startswith("#") else "FAIL",
    ...
    for s in sorted(forb):
        if s in seen:
            add("FAIL", f"section '{s}' forbidden",
                "trigger is false in services*.json — README and metadata disagree")
        else:
    ...
    add("PASS" if idx == sorted(idx) else "FAIL", "section order",
    ...
    if "About" in seen:
        ...
        add("PASS" if "Upstream docs" in seen else "FAIL",
            "Upstream docs present when About exists")

    # -- table-key parity --
    if svc.get("invoke"):
        want = set(svc["invoke"].keys())
        got = set(table_col(hand, "Connections"))
        add("PASS" if want <= got else "FAIL", "Connections rows match invoke keys",
    ...
    if svc.get("lanes") and "Lanes" in seen:
        ...
        add("PASS" if want <= got else "FAIL", "Lanes rows match declared lanes",
    ...
    if "Example pipelines" in seen:
        ...
        add("PASS" if body and "→" in body.group(1) else "FAIL",
            "Example pipelines has >=1 flow", "no `a → b` pipeline shape found")
...
def main():
    ...
    if args[0] == "--all":
        dirs = sorted(p for p in Path(args[1]).iterdir()
                      if p.is_dir() and any(p.glob("services*.json")))
    else:
    ...
    if any_fail:
        print("\nSchema: docs/internal/node-readme-schema.md")
