...
const { execCommand, getState, setState, PROJECT_ROOT, readDir, readFile, exists } = require('./lib');
...
async function checkDependencies(options) {
	const root = options.overlayRoot && (await exists(path.join(options.overlayRoot, 'package.json'))) ? options.overlayRoot : PROJECT_ROOT;
