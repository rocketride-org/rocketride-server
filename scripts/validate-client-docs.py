...
docs/internal/client-readme-schema.md.
...
PARITY_SECTIONS = ["RocketRideClient", "DataPipe", "Question", "Answer",
                   "Types", "Exceptions"]
...
def harvest(section_text: str):
    ...
    """
    section_text = re.sub(r"^### Constructor.*?(?=^### |\Z)", "",
                          section_text, flags=re.M | re.S)
    out = {}
...
def main():
    ...
    for lang, text in docs.items():
        ...
        for h in heads:
            if h not in allowed:
                other = [l for l, ss in LANG_SECTIONS.items() if h in ss]
                if other:
                    failures.append(
                        f"{lang}: section '## {h}' belongs to {other[0]} only")
                else:
        ...
        for extra, after in LANG_POSITION.items():
            if extra in heads and after in heads:
                if heads.index(extra) != heads.index(after) + 1:
                    failures.append(
                        f"{lang}: '## {extra}' must directly follow '## {after}'")
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        print("\nSchema: docs/internal/client-readme-schema.md")
        sys.exit(1)
    print("ok: client docs conform to the schema "
