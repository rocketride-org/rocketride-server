...
interface HashViewProps {
	hashes: ProcessedResults['hashes'];
	compareMode: boolean;
	setRef?: (filename: string, element: HTMLDivElement | null) => void;
}

export const HashView: React.FC<HashViewProps> = ({ hashes, compareMode, setRef }) => {
	if (hashes.length === 0) {
		return (
			<div className="tab-content">
				<div className="no-content">
					<Fingerprint className="w-12 h-12 text-gray-300" />
					<p>No fingerprints found in the processed files.</p>
				</div>
			</div>
		);
	}

	const renderHash = (content: any) => {
		if (!content) {
									...
									<div key={contentIndex} className="compare-column">
										...
										<div className="content-item">
											<div className="answer-content">
												<Fingerprint className="answer-icon" />
												{renderHash(block.content)}
											</div>
										</div>
									</div>
								))}
