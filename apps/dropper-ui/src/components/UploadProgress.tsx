		...
		<div className="upload-progress-panel">
			...
			<div className="upload-progress-list">
				{uploadProgress.map((progress) => {
					...
					const percentage = progress.file_size > 0 ? (progress.bytes_sent / progress.file_size) * 100 : 0;

					return (
						<div key={progress.filepath} className="upload-progress-item">
							{/* Filename section - 25% width */}
							<div className="upload-progress-filename">{progress.filepath}</div>

							{/* Progress bar section - 50% width */}
							<div className="upload-progress-bar-wrapper">
								<div className="upload-progress-bar-container">
									<div className="upload-progress-bar" style={{ width: `${percentage}%` }} role="progressbar" aria-label={`Upload progress for ${progress.filepath}: ${percentage.toFixed(0)}%`} />
								</div>
							</div>
