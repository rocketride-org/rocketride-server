 ...
 * ```
 */
export const ImageCarousel: React.FC<ImageCarouselProps> = ({ images, initialIndex, onClose }) => {
	...
	return (
		...
		<div className="carousel-overlay" onClick={onClose}>
			...
			<div className="carousel-content" onClick={(e) => e.stopPropagation()} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} style={{ cursor: scale > MIN_SCALE ? (isDragging ? 'grabbing' : 'grab') : 'default' }}>
				{/* Zoom controls in top-left corner */}
				<div className="carousel-zoom-controls">
					<button className="carousel-zoom-btn" onClick={zoomOut} disabled={scale <= MIN_SCALE} aria-label="Zoom out" title="Zoom out (-)">
						<ZoomOut className="w-5 h-5" />
					</button>
					<span className="carousel-zoom-level">{Math.round(scale * 100)}%</span>
					<button className="carousel-zoom-btn" onClick={zoomIn} disabled={scale >= MAX_SCALE} aria-label="Zoom in" title="Zoom in (+)">
				...
				{currentIndex > 0 && (
					<button className="carousel-nav carousel-prev" onClick={goToPrevious} aria-label="Previous image">
				...
				<div ref={imageRef} className="carousel-image-container">
					<img
