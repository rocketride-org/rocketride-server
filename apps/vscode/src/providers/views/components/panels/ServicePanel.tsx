		...
		<>
			...
			{onTestConnection && status.state !== 'not-installed' && (
				<div style={{ marginTop: 12 }}>
					<button type="button" onClick={onTestConnection} disabled={status.state !== 'running' || busy} style={secondaryBtnStyle(hoveredBtn === `${idPrefix}-service-test`, status.state !== 'running' || busy)} onMouseEnter={() => setHoveredBtn(`${idPrefix}-service-test`)} onMouseLeave={() => setHoveredBtn(null)}>
						Test Connection
					</button>
				</div>
			)}
