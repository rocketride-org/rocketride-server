...
export class DeployProvider {
	...
	private writeServiceConfig(info: { versionSpec: string; version: string; publishedAt: string }): void {
		const configDir = path.dirname(DeployProvider.CONFIG_PATH);
		fs.mkdirSync(configDir, { recursive: true });
		fs.writeFileSync(
			DeployProvider.CONFIG_PATH,
			JSON.stringify(
				{
					...info,
	...
	private readServiceConfig(): { versionSpec: string; version: string; publishedAt: string } | null {
		try {
			if (fs.existsSync(DeployProvider.CONFIG_PATH)) {
				return JSON.parse(fs.readFileSync(DeployProvider.CONFIG_PATH, 'utf8'));
			}
		} catch {
			/* corrupt */
		}
	...
	private writeDockerConfig(info: { versionSpec: string; imageTag: string }): void {
		const configDir = path.dirname(DeployProvider.DOCKER_CONFIG_PATH);
		fs.mkdirSync(configDir, { recursive: true });
		fs.writeFileSync(
			DeployProvider.DOCKER_CONFIG_PATH,
			JSON.stringify(
				{
					...info,
					installedAt: new Date().toISOString(),
	...
	private readDockerConfig(): { versionSpec: string; imageTag: string } | null {
		try {
			if (fs.existsSync(DeployProvider.DOCKER_CONFIG_PATH)) {
				return JSON.parse(fs.readFileSync(DeployProvider.DOCKER_CONFIG_PATH, 'utf8'));
			}
		} catch {
			/* corrupt */
		}
	...
	private deleteDockerConfig(): void {
		try {
			if (fs.existsSync(DeployProvider.DOCKER_CONFIG_PATH)) {
				fs.unlinkSync(DeployProvider.DOCKER_CONFIG_PATH);
			}
		} catch {
			/* best effort */
		}
