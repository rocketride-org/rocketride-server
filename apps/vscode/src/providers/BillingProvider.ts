...
export class BillingProvider {
	...
	/**
	 * Creates a new BillingProvider and registers the open command.
	 *
	...
	public show(): void {
		...
		panel.webview.onDidReceiveMessage(async (message) => {
			try {
				...
				console.error(`[BillingProvider] Message handling error: ${error}`);
			}
		});
	...
	private async handleCancel(appId: string): Promise<void> {
		...
		try {
			...
			console.error(`[BillingProvider] Failed to cancel subscription: ${error}`);
			if (this.panel) {
				await this.panel.webview.postMessage({
					type: 'billing:data',
					subscriptions: [],
	...
	private async handlePortal(): Promise<void> {
		...
		try {
			...
			console.error(`[BillingProvider] Failed to open billing portal: ${error}`);
		}
	}
	...
	private async handleBuyCredits(packId: string): Promise<void> {
		...
		try {
			...
			console.error(`[BillingProvider] Failed to create credit checkout: ${error}`);
		}
	}
	...
	private setupEventListeners(): void {
		const connectionStateListener = this.connectionManager.on('shell:statusChange', (status: ConnectionStatus) => {
			this.handleConnectionStateChange(status).catch((error) => {
				console.error(`[BillingProvider] Connection state change error: ${error}`);
			});
		});
