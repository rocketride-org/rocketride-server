...
/**
 * PageStatusProvider — Webview panel for monitoring "Other" (unknown) tasks.
 *
 * Opens a full ProjectView panel (same as PageProjectProvider) for tasks that
	...
	public async show(projectId: string, sourceId: string, displayName: string): Promise<void> {
		...
		if (client) {
			try {
				...
				console.error('[PageStatusProvider] Could not fetch task pipeline:', err);
			}
		}
		...
		panel.webview.onDidReceiveMessage(async (message) => {
			try {
				switch (message.type) {
					case 'view:ready': {
						// Send project:load exactly as PageProjectProvider does
