...
| `rocketride.pipelineReplicas`        | `number`   | `0`                              | Task replicas (engine subprocesses, each a full model copy) per pipeline run, 0-32. `0` = server default (normally 1)                     |
| `rocketride.pipelineTorchThreads`    | `number`   | `0`                              | BLAS/OMP threads per replica. `0` = auto: `cores / replicas` when replicas > 1, nothing injected at 1                                     |
