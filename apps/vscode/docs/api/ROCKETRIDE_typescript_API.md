...
const client = new RocketRideClient({
    ...
    reconnectDelay: 2000,       // Wait 2 seconds before reconnection attempts (default: 1000)
...
// Upload files in parallel (max 64 concurrent by default)
const results = await client.sendFiles(
    ...
    'task-token',
    10  // Optional: max concurrent uploads (default: 64)
);
...
- `persist?: boolean` - Enable automatic reconnection (default: false)
...
**Options:**
- `pipeline?: PipelineConfig` - Pipeline configuration object
- `filepath?: string` - Path to JSON file containing pipeline configuration (Node.js only)
...
Upload multiple files in parallel with configurable concurrency (default: 64 concurrent uploads).

**Important:** Use this method with pipelines that have `webhook`, `filesys`, or `dropper` as the source component for document processing. For chat/Q&A systems, use `chat()` method with a `chat` source component instead.
...
interface PipelineConfig {
  pipeline: {
    components: Array<{
      ...
      input?: Array<{          // Input connections from other components
...
const result = await client.use({ filepath: 'chat_pipeline.json' });
...
async function processDocuments() {
    ...
    try {
        ...
        const result = await client.use({ filepath: 'document_analyzer.json' });
        const token = result.token;
...
async function streamSensorData(dataGenerator: AsyncIterable<SensorReading>) {
    ...
    try {
        ...
        const result = await client.use({ filepath: 'sensor_processor.json' });
        const token = result.token;
...
- File uploads are parallelized (configurable, default 64 concurrent transfers)
- Adjust `maxConcurrent` parameter in `sendFiles()` based on your network and server capacity
