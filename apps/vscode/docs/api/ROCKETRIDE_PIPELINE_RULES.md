...
- Typically a source-type component (webhook, chat, filesys, etc.)
...
- Examples: webhook, chat, filesys, dropper
...
```
chat → embedding → vector_db → llm → response
```
**Use Case:** Any conversational interface (web, console, API, mobile)  
**Client Method:** `client.chat(token, question)`  
...
1. **Start Minimal**: Begin with source → response
...
- **Official Docs**: `.cursor/doc/rocketride.README.md`
- **SDK Documentation**: `.cursor/doc/rocketride.python.README.md` and `.cursor/doc/rocketride.typescript.README.md`
