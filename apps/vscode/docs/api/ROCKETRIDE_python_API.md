...
##### `async pipe(token: str, objinfo: Dict[str, Any] = {}, mimetype: str = None, provider: str = None) -> DataPipe`
...
async def stream_sensor_data(data_generator):
    async with RocketRideClient() as client:  # Configuration from .env
        result = await client.use(filepath='sensor_processor.json')
        token = result['token']
...
- Adjust `max_concurrent` parameter in `send_files()` based on your network and server capacity
